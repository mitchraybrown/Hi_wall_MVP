...
import { fmt, TRAFFIC_LEVELS, COORDS } from '../lib/pricing'
...
function WallCard({ wall: w, onClick }) {
  ...
  return <div className="ch" onClick={onClick} style={{background:'var(--wh)',borderRadius:'var(--rl)',overflow:'hidden',cursor:'pointer',border:'1px solid var(--ln)',position:'relative'}}>
    ...
    <div style={{padding:'12px 14px 14px'}}><h3 style={{fontSize:15,fontWeight:600,marginBottom:3}}>{w.title}</h3><div style={{fontSize:12,color:'var(--mu)',marginBottom:6}}>{w.neighborhood} • {w.width_m}m × {w.height_m}m</div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingTop:9,borderTop:'1px solid var(--ln)'}}><span style={{fontFamily:'var(--fd)',fontSize:17,fontWeight:700,color:'var(--co)'}}>{fmt(w.price_total)}</span><span style={{fontSize:11,color:'var(--mu)'}}>/{w.duration_months}mo</span></div>
    </div>
  </div>
}
  ...
  return <Overlay onClose={onClose}><div style={{background:'var(--wh)',borderRadius:'var(--rl)',maxWidth:840,width:'95vw',maxHeight:'90vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,.15)'}}>
    ...
    <div style={{padding:'20px 24px 24px'}}>
      <div style={{display:'flex',justifyContent:'space-between',flexWrap:'wrap',gap:12,marginBottom:4}}>
        ...
        <div style={{background:'var(--col)',padding:'11px 18px',borderRadius:'var(--r)',textAlign:'right'}}><div style={{fontFamily:'var(--fd)',fontSize:24,fontWeight:700,color:'var(--co)'}}>{fmt(w.price_total)}</div><div style={{fontSize:12,color:'var(--sl)'}}>for {w.duration_months} mo</div></div>
      </div>
...
// ── How It Works + Creative Fit ──────────────────────────────────────────
function HowItWorks() {
  return <Card className="au d2" style={{padding:0,marginBottom:24,overflow:'hidden'}}>
    <div style={{padding:'26px 30px 24px'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4,flexWrap:'wrap',gap:8}}>
        ...
        <span style={{fontSize:12,color:'var(--mu)',fontStyle:'italic'}}>Inventory → Verification → Booking → Delivery</span>
      </div>
    ...
    <div style={{background:'linear-gradient(135deg,#1A1A2E,#2D2B55)',padding:'24px 30px 26px'}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}><span style={{fontSize:16}}>🎨</span><h4 style={{fontFamily:'var(--fd)',fontSize:16,fontWeight:700,color:'#fff'}}>Creative Fit Standard</h4></div>
      <p style={{fontSize:12,color:'rgba(255,255,255,.45)',marginBottom:16,maxWidth:460}}>Every campaign is measured against three checks — balancing authentic art with effective advertising.</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:12}}>
        {[{t:'Street Fit',d:'Would locals accept this as enhancing the streetscape?',icon:'🏘️'},{t:'Brand Fit',d:'Is the brand presence clear enough to justify spend?',icon:'📊'},{t:'Artist Fit',d:'Does the execution allow genuine craft and pride?',icon:'🖌️'}].map((c,i)=><div key={i} style={{background:'rgba(255,255,255,.06)',borderRadius:12,padding:'16px 14px',border:'1px solid rgba(255,255,255,.08)'}}>
          <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:8}}><span style={{fontSize:14}}>{c.icon}</span><span style={{fontFamily:'var(--fd)',fontSize:14,fontWeight:700,color:'var(--co)'}}>{c.t}</span></div>
          <p style={{fontSize:12,color:'rgba(255,255,255,.55)',lineHeight:1.6}}>{c.d}</p>
        </div>)}
      </div>
    </div>
  </Card>
...
export default function BrowsePage({ toast }) {
  ...
  const [sel, sSel] = useState(null); const [enquiry, sEnq] = useState(null); const [fA, sFA] = useState(''); const [hov, sHov] = useState(null)
  ...
  let list = walls.filter(w => {
    ...
    return (!s || w.title.toLowerCase().includes(s) || w.neighborhood.toLowerCase().includes(s))
      ...
      && (!fA || (fA === 'available' && w.availability_status !== 'booked') || (fA === 'booked' && w.availability_status === 'booked'))
  })
          ...
          <div style={{maxHeight:'calc(100vh - 180px)',overflowY:'auto',display:'flex',flexDirection:'column',gap:10,paddingRight:6}}>
            {list.map(w => <Card key={w.id} onClick={()=>sSel(w)} onMouseEnter={()=>sHov(w.id)} onMouseLeave={()=>sHov(null)} style={{padding:12,display:'flex',gap:12,cursor:'pointer',border:hov===w.id?'1.5px solid var(--co)':'1px solid var(--ln)',position:'relative'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontFamily:'var(--fd)',fontSize:16,fontWeight:700,color:'var(--co)'}}>{fmt(w.price_total)}</span><span style={{fontSize:10,color:'var(--mu)'}}>/{w.duration_months}mo</span></div>
              </div>
            </Card>)}
          </div>
